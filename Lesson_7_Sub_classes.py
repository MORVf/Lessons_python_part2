...
n = int(input()) # считываем количество строк для ввода классов и наследований
dict_classes = {} #словарь для хранения классов и наследований


def is_parent(parent, heir):  #функция, определяющая наличие связи предок-наследник у классов
    global is_heir   #глобальная переменная "найден ли наследник"
    ...
    if heir in dict_classes[parent]:   #если наследник есть в первичных подклассах родителя
        ...
        if parent == is_root_parent and is_heir is False:    #если во всех подклассах подклассов предка не нашлось наследника, то
            return print('No')    #выводим Нет
    elif is_end_cycle is True:   #если подклассов нет и наследник не найден, то
        return print('No')   #выводим сразу Нет


def add_subclass_in_parent():    #функция добавления классов и наследований
...
q = int(input())   #считываем количество строк, в которых будет проверяться предок-наследник
